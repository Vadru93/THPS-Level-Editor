...
/*
	...
	while(p->NextCustomCell)
	{
		...
		for(DWORD i=0;i<p->Item.Size;i++) Store(*Addy++);
...
bool CustomArray::PushAddress()
{
	...
	if((mNbPushedAddies+1)>mNbAllocatedAddies)
	{
		...
		unsigned int NewSize = mNbAllocatedAddies ? mNbAllocatedAddies * 2 : 1;
...
// Output	:	-
// Return	:	unsigned int	= size in bytes, or 0 if file doesn't exist
// Exception:	-
// Remark	:	-
#ifndef SEEK_END
#define SEEK_END 2
#endif
unsigned int CustomArray::FileSize(const char* name)
{
	FILE* File = fopen(name, "rb");
	if (!File) return 0;
	fseek(File, 0, SEEK_END);
	unsigned int eof_ftell = ftell(File);
	fclose(File);
	return eof_ftell;
}
