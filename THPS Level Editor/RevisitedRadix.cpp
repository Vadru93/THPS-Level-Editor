...
RadixSorter& RadixSorter::Sort(unsigned int* input, unsigned int nb, bool signedvalues)
{
	...
	// Clear counters
	ZeroMemory(mHistogram, 256*4*sizeof(unsigned int));
	...
	ubyte* p = (ubyte*)input;
	ubyte* pe = &p[nb*4];
	unsigned int* h0= &mHistogram[0];						// Histogram for first pass (LSB)
	unsigned int* h1= &mHistogram[256];					// Histogram for second pass
	unsigned int* h2= &mHistogram[512];					// Histogram for third pass
	unsigned int* h3= &mHistogram[768];					// Histogram for last pass (MSB)
	if(!signedvalues)
	{
		// Temporal coherence
		unsigned int PrevVal = input[mIndices[0]];

		while(p!=pe)
		{
			// Temporal coherence
			unsigned int Val = input[*Indices++];				// Read input buffer in previous sorted order
	...
	if(signedvalues)
	{
		...
		for(unsigned int i=128;i<256;i++)	NbNegativeValues += h3[i];	// 768 for last histogram, 128 for negative part
	}
	...
	for(unsigned int j=0;j<4;j++)
	{
		...
		unsigned int* CurCount = &mHistogram[j<<8];
		...
		if(PerformPass)
		{
			...
			else
			{
				...
				for(unsigned int i=1;i<128;i++)		mOffset[i] = mOffset[i-1] + CurCount[i-1];	// 1 to 128 for positive numbers

				// Fixing the wrong place for negative values
				mOffset[128] = 0;
				for(int i=129;i<256;i++)	mOffset[i] = mOffset[i-1] + CurCount[i-1];
			}
	...
	for(unsigned int j=0;j<4;j++)
	{
		...
		if(PerformPass)
		{
			...
			else
			{
				...
				mOffset[255] = 0;
				for(int i=0;i<127;i++)		mOffset[254-i] = mOffset[255-i] + CurCount[255-i];	// Fixing the wrong order for negative values
				for(int i=128;i<256;i++)	mOffset[i] += CurCount[i];							// Fixing the wrong place for negative values

				// Perform Radix Sort
				for(int i=0;i<nb;i++)
				{
					unsigned int Radix = input[mIndices[i]]>>24;								// Radix byte, same as above. AND is useless here (unsigned int).
...
// Input	:	-
// Output	:	-
// Return	:	unsigned int,	memory used in bytes
// Exception:	-
// Remark	:	-
unsigned int RadixSorter::GetUsedRam()
{
	unsigned int UsedRam = 0;
	UsedRam += 256*4*sizeof(unsigned int);			// Histograms
	UsedRam += 256*sizeof(unsigned int);				// Offsets
	UsedRam += 2*mCurrentSize*sizeof(unsigned int);	// 2 lists of indices
	return UsedRam;
}
