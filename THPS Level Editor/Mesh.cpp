...
__declspec (noalias) void Mesh::SetPositionMeshOnly(const D3DXVECTOR3 const* __restrict position, const D3DXVECTOR3 const* __restrict angle)
{
	D3DXVECTOR3 center = GetCenter();
	char text[256] = "";
	sprintf(text, "Mesh: OldPos %f %f %f new %f %f %f", center.x, center.y, center.z, position->x, position->y, position->z);
	...
	*(D3DXVECTOR3*)&bbox.Min = *position - (*(D3DXVECTOR3*)&bbox.Min + center);
	...
	D3DXMATRIX result;
	D3DXMatrixRotationYawPitchRoll(&rotation, angle->y - D3DX_PI / 2.0f/*-angle->y + D3DX_PI/2.0f*/, angle->x - D3DX_PI / 2.0f, angle->z  +D3DX_PI / 2.0f);
...
__declspec (noalias) void Mesh::SetPosition(const D3DXVECTOR3 const* __restrict position, const D3DXVECTOR3 const* __restrict angle)
{
	...
	*(D3DXVECTOR3*)&bbox.Min = *position - (*(D3DXVECTOR3*)&bbox.Min + center);
	...
	if (collision.get())
		collision->SetPosition(position, angle, center);
}

__declspec (noalias) void Mesh::SetAngles(const D3DXVECTOR3 const* __restrict angle)
{
	D3DXVECTOR3 center = GetCenter();
	//MessageBox(0, text, "SETPOS", 0);
	*(D3DXVECTOR3*)&bbox.Max = (*(D3DXVECTOR3*)&bbox.Max - center);
	*(D3DXVECTOR3*)&bbox.Min = (*(D3DXVECTOR3*)&bbox.Min + center);
	D3DXMATRIX rotation;
	D3DXMATRIX translation;
	D3DXMATRIX result;
	D3DXMatrixRotationYawPitchRoll(&rotation, angle->y - D3DX_PI / 2.0f/*-angle->y + D3DX_PI/2.0f*/, angle->x - D3DX_PI / 2.0f, angle->z + D3DX_PI / 2.0f);
	D3DXVec3TransformCoord((D3DXVECTOR3*)&bbox.Max, (D3DXVECTOR3*)&bbox.Max, &rotation);
	D3DXVec3TransformCoord((D3DXVECTOR3*)&bbox.Min, (D3DXVECTOR3*)&bbox.Min, &rotation);
	if (bbox.Min.x > bbox.Max.x)
	{
		const float temp = bbox.Max.x;
		bbox.Max.x = bbox.Min.x;
