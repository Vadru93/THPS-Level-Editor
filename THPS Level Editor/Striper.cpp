...
bool Striper::Compute(STRIPERRESULT& result)
{
	...
	unsigned int* Connectivity	= new unsigned int[mAdj->mNbFaces];	if(!Connectivity)	return false;
	...
	ZeroMemory(Connectivity, mAdj->mNbFaces*sizeof(unsigned int));
	if(mSGIAlgorithm)
	{
		// Compute number of adjacent triangles for each face
		for(unsigned int i=0;i<mAdj->mNbFaces;i++)
		{
		...
		RadixSorter RS;
		unsigned int* Sorted = RS.Sort(Connectivity, mAdj->mNbFaces).GetIndices();

		// The sorted indices become the order of insertion in the strips
		CopyMemory(Connectivity, Sorted, mAdj->mNbFaces*sizeof(unsigned int));
	}
	else
	{
		// Default order
		for(unsigned int i=0;i<mAdj->mNbFaces;i++)	Connectivity[i] = i;
	}

	mNbStrips			= 0;	// #strips created
	unsigned int TotalNbFaces	= 0;	// #faces already transformed into strips
	unsigned int Index		= 0;	// Index of first face
	...
	result.StripLengths	= (unsigned int*)	mStripLengths	->Collapse();
...
// Output	:	-
// Return	:	unsigned int,		the #faces included in the strip
// Exception:	-
// Remark	:	mStripLengths and mStripRuns are filled with strip data
unsigned int Striper::ComputeBestStrip(unsigned int face)
{
	unsigned int* Strip[3];		// Strips computed in the 3 possible directions
	unsigned int* Faces[3];		// Faces involved in the 3 previous strips
	unsigned int Length[3];		// Lengths of the 3 previous strips

	unsigned int FirstLength[3];	// Lengths of the first parts of the strips are saved for culling
	...
	for(unsigned int j=0;j<3;j++)
	{
		...
		Strip[j] = new unsigned int[mAdj->mNbFaces+2+1+2];	// max possible length is NbFaces+2, 1 more if the first index gets replicated
		Faces[j] = new unsigned int[mAdj->mNbFaces+2];
		FillMemory(Strip[j], (mAdj->mNbFaces+2+1+2)*sizeof(unsigned int), 0xff);
		FillMemory(Faces[j], (mAdj->mNbFaces+2)*sizeof(unsigned int), 0xff);
...
//		if(j==1)	FirstLength[j]++;	// ...because the first face is written in reverse order for j==1
		...
		unsigned int ExtraLength = TrackStrip(face, NewRef0, NewRef1, &Strip[j][Length[j]-3], &Faces[j][Length[j]-3], Tags);
	...
	// Update global tags
	for(int j=0;j<Longest-2;j++)	mTags[Faces[Best][j]] = true;
	...
	for(int j=0;j<Longest;j++)
	{
		...
		if(mAskForWords)	mStripRuns->Store((unsigned short)Ref);	// Saves word reference
...
// Return	:	unsigned int,				the strip length
// Exception:	-
// Remark	:	-
unsigned int Striper::TrackStrip(unsigned int face, unsigned int oldest, unsigned int middle, unsigned int* strip, unsigned int* faces, bool* tags)
{
	unsigned int Length = 2;														// Initial length is 2 since we have 2 indices in input
	...
	while(DoTheStrip)
	{
		unsigned int Newest = mAdj->mFaces[face].OppositeVertex(oldest, middle);	// Get the third index of a face given two of them
		...
		*faces++ = face;													// ...keep track of the face,...
		...
		unsigned int Link = mAdj->mFaces[face].ATri[CurEdge];						// ...and use it to catch the link to adjacent face.
...
bool Striper::ConnectAllStrips(STRIPERRESULT& result)
{
	...
	mTotalLength	= 0;
	unsigned short* wrefs	= mAskForWords ? (unsigned short*)result.StripRuns : NULL;
	unsigned int* drefs	= mAskForWords ? NULL : (unsigned int*)result.StripRuns;

	// Loop over strips and link them together
	for(unsigned int k=0;k<result.NbStrips;k++)
	{
		...
		if(k)
		{
			...
			unsigned int LastRef	= drefs ? drefs[-1] : (unsigned int)wrefs[-1];
			unsigned int FirstRef	= drefs ? drefs[0] : (unsigned int)wrefs[0];
			if(mAskForWords)	mSingleStrip->Store((unsigned short)LastRef).Store((unsigned short)FirstRef);
			...
			if(mOneSided)
			{
				...
				if(mTotalLength&1)
				{
					...
					unsigned int SecondRef = drefs ? drefs[1] : (unsigned int)wrefs[1];
					if(FirstRef!=SecondRef)
					{
						if(mAskForWords)	mSingleStrip->Store((unsigned short)FirstRef);
		...
		// Copy strip
		for(unsigned int j=0;j<result.StripLengths[k];j++)
		{
			unsigned int Ref = drefs ? drefs[j] : (unsigned int)wrefs[j];
			if(mAskForWords)	mSingleStrip->Store((unsigned short)Ref);
