...
DWORD GetGapType(WORD id, char* outText)
{
  ...
  if(id==0x5785) MessageBox(0,"returning 0",nullptr,0); return 0;
}
...
struct ObjInfo
{
  ...
  ObjInfo(DWORD chc, DWORD id, DWORD numVerts, DWORD verts, DWORD numInd, DWORD ind)
  {
    checksum=chc;
    matIndex=id;
...
void Th2Scene::ProcessXBOXMaterials(DWORD ptr, bool sky)
{
  ...
  for(DWORD i=0; i<numObjects; i++)
  {
    ...
    if(numMaterials==1)
      objInfo.push_back(ObjInfo(checksum, id, numVertices, pObject, numIndices, pObject+numVertices*sizeof(XboxVertex)));
...
void Th2Scene::ProcessXBOXTextures(DWORD ptr)
{
  bool merging=false;
  DWORD pFile = ptr;
  ptr+=8;
  ...
  {
    merging = true;
    texFiles.fileNames = (char**)realloc(texFiles.fileNames, (texFiles.numFiles+numTextures)*4);
  }
  ptr+=4;
  static char imageName[MAX_PATH] = "";
  GetEnvironmentVariable("TEMP",imageName, MAX_PATH);
...
void Th2Scene::ProcessTextures(DWORD ptr, bool sky)
{
  ...
  if(!sky)
  {
    ...
    {
      merging = true;
      texFiles.fileNames = (char**)realloc(texFiles.fileNames, (texFiles.numFiles+numImages)*4);
    }
  }
...
#pragma pack(pop)
  ...
  if(f)
  {
    ...
    ProcessXBOXTextures((DWORD)(pFile));
  ...
  if(f)
  {
    ...
    for(DWORD i=0; i<numMeshes; i++)
    {
      ...
      /*vertices.resize(meshes[i]->numVertices);
      std::copy(meshes[i]->vertices, meshes[i]->vertices+meshes[i]->numVertices, vertices.begin());*/
      /*for(DWORD j=0; j<meshes[i]->numVertices; j++)
      {
      ...
      for(DWORD j=0; j<meshes[i]->numQuads; j++)
      {
        ...
        if(!((GetBit(meshes[i]->tristrip[j]->flags,7) && !GetBit(meshes[i]->tristrip[j]->flags,6))/* ||  meshes[i]->tristrip[j]->collFlags & 0x2*/))
        {
          ...
          if(((GetBit(meshes[i]->tristrip[j]->flag,0) || GetBit(meshes[i]->tristrip[j]->flag,3) || GetBit(meshes[i]->tristrip[j]->flag,1) ) &&  meshes[i]->tristrip[j]->flag != 128 && meshes[i]->tristrip[j]->flag != 0x90 && meshes[i]->tristrip[j]->flag != 0 && meshes[i]->tristrip[j]->flag != 0x10 && (meshes[i]->tristrip[j]->flags & 0xFF0000) != 0x100000))//if(GetBit(meshes[i]->tristrip[j]->flags,0)/* || GetBit(meshes[i]->tristrip[j]->flags,1)*/)//meshes[i]->tristrip[j]->flag != 128 && meshes[i]->tristrip[j]->flag != 0x90 && meshes[i]->tristrip[j]->flag != 0 && meshes[i]->tristrip[j]->flag != 0x10)
          {
            ...
            if(tmpMat==NULL)
            {
              ...
              /*meshes[i]->tristrip[j]->matIndex = matList.GetNumMaterials();
              meshes[i]->tristrip[j]->matIndex--;
              tmpMat = matList.GetMaterial(meshes[i]->tristrip[j]->matIndex);*/
            }
            if(tmpMat)
            {
              /*if(!GetBit(meshes[i]->tristrip[j]->flags,5))
              {
              float width = (float)tmpMat->GetTexture(0)->GetWidth();
              float height = (float)tmpMat->GetTexture(0)->GetHeight();

              uv[0] = (float)(meshes[i]->tristrip[j]->uv[0])/(4096.0f);
              uv[2] = (float)(meshes[i]->tristrip[j]->uv[1])/(4096.0f);
              uv[4] = (float)(meshes[i]->tristrip[j]->uv[2])/(4096.0f);
              uv[6] = (float)(meshes[i]->tristrip[j]->uv[3])/(4096.0f);
              uv[1] = (float)(meshes[i]->tristrip[j]->uv[4])/(4096.0f);
              uv[3] = (float)(meshes[i]->tristrip[j]->uv[5])/(4096.0f);
              uv[5] = (float)(meshes[i]->tristrip[j]->uv[6])/(4096.0f);
              uv[7] = (float)(meshes[i]->tristrip[j]->uv[7])/(4096.0f);
              ...
              //if(meshes[i]->tristrip[j]->uv[0] & 0xFF00)
              uv[0] = (float)(meshes[i]->tristrip[j]->uv[0])/(512.0f);
              /*else
              uv[0] = (float)(meshes[i]->tristrip[j]->uv[0])/(width*4.0f);*/
              //if(meshes[i]->tristrip[j]->uv[1] & 0xFF00)
              uv[2] = (float)(meshes[i]->tristrip[j]->uv[1])/(512.0f);
              /*else
              uv[2] = (float)(meshes[i]->tristrip[j]->uv[1])/(width*4.0f);*/
              //if(meshes[i]->tristrip[j]->uv[2] & 0xFF00)
              uv[4] = (float)(meshes[i]->tristrip[j]->uv[2])/(512.0f);
              /*else
              uv[4] = (float)(meshes[i]->tristrip[j]->uv[2])/(width*4.0f);*/
              //if(meshes[i]->tristrip[j]->uv[3] & 0xFF00)
              uv[6] = (float)(meshes[i]->tristrip[j]->uv[3])/(512.0f);
              /*else
              uv[6] = (float)(meshes[i]->tristrip[j]->uv[3])/(width*4.0f);*/
              //if(meshes[i]->tristrip[j]->uv[4] & 0xFF00)
              uv[1] = (float)(meshes[i]->tristrip[j]->uv[4])/(512.0f);
              /*else
              uv[1] = (float)(meshes[i]->tristrip[j]->uv[4])/(height*4.0f);*/
              //if(meshes[i]->tristrip[j]->uv[5] & 0xFF00)
              uv[3] = (float)(meshes[i]->tristrip[j]->uv[5])/(512.0f);
              /*else
              uv[3] = (float)(meshes[i]->tristrip[j]->uv[5])/(height*4.0f);*/
              //if(meshes[i]->tristrip[j]->uv[6])
              uv[5] = (float)(meshes[i]->tristrip[j]->uv[6])/(512.0f);
              /*else
              uv[5] = (float)(meshes[i]->tristrip[j]->uv[6])/(height*4.0f);*/
              //if(meshes[i]->tristrip[j]->uv[7] & 0xFF00)
          ...
          else
          {
            ...
            /*if(tmpMat)
            {
            tmpMat->drawOrder += (float)splitCount;
            }*/
          ...
          /*if(a<matSplit[splitCount].type)
          matSplit[splitCount].type = a;
          if(b<matSplit[splitCount].type)
          matSplit[splitCount].type = b;
          if(c<matSplit[splitCount].type)
          matSplit[splitCount].type = c;*/
          ...
          faceInfo.flags &= ~0x0200;
          if(meshes[i]->tristrip[j]->collFlags & 0x0100 || !meshes[i]->tristrip[j]->collFlags & 0x0080)
          faceInfo.flags |= 0x0004;
          faceInfo.flags |= 0x0400;
          faceInfo.flags &= ~0x0010;
          faceInfo.flags |= 0x0001;
          }

          if(meshes[i]->tristrip[j]->collFlags & 1)
          {
          faceInfo.flags &= ~0x0004;
          faceInfo.flags &= ~0x0001;
          faceInfo.flags |= 0x0010;
          }
          else if(meshes[i]->tristrip[j]->collFlags & 2)
          {
          faceInfo.flags &= ~0x0004;
          faceInfo.flags &= ~0x0001;
          faceInfo.flags |= 0x0010;
          ...
          if(meshes[i]->tristrip[j]->collFlags & 0x0040)
          {
          faceInfo.flags |= 0x0008;
          faceInfo.flags &= ~0x0004;
          faceInfo.flags &= ~0x0001;
          faceInfo.flags |= 0x0010;
          }
          else if(meshes[i]->tristrip[j]->collFlags & 0x0020)
          {
          faceInfo.flags &= ~0x0010;
          faceInfo.flags &= ~0x0004;
          faceInfo.flags |= 0x0008;
          ...
          if(!GetBit(meshes[i]->tristrip[j]->flag, 4))
          {
            /*if(d<matSplit[splitCount].type)
            matSplit[splitCount].type = d;*/
          ...
          if((GetBit(meshes[i]->tristrip[j]->flags,7) && GetBit(meshes[i]->tristrip[j]->flags,6)) || (!GetBit(meshes[i]->tristrip[j]->flags,7) && GetBit(meshes[i]->tristrip[j]->flags,6)))
          {
            ...
            //Material* tmpMat = (Material*)matList.GetMaterial(meshes[i]->tristrip[j]->matIndex);

            if(tmpMat)
            {
              tmpMat->transparent=true;
              tmpMat->drawOrder=1501.0f;
              //tmpMat->drawOrder = 100.0f;
            }
          }
        ...
        printf("meshnum %d faceindex0 %d faceindex1 %d faceindex2 %d faceindex3 %d numverts %d\n",i,meshes[i]->tristrip[j]->a,meshes[i]->tristrip[j]->b,meshes[i]->tristrip[j]->c,meshes[i]->tristrip[j]->d,meshes[i]->numVertices);
        */
        ptr+=16;
        ...
        /*else if(GetBit(meshes[i]->tristrip[j]->flag, 1))
        {
        MessageBox(0, "E", "",0);
        ptr+=12;
...
doch:
        ...
        //matSplit[0].numIndices = ddmObj->numIndices;
        //matSplit[0].Indices.resize(ddmObj->numIndices);
        //matSplit[0].Indices.assign(ddmObj->indices, ddmObj->indices+ddmObj->numIndices);
        vector<SimpleFace> polygons;
        matSplit[0].Indices.clear();
        DWORD numTris = ddmObj->numIndices-2;

        for(DWORD j=0; j<numTris; j++)
        {
          WORD a= ddmObj->indices[j];
          WORD b= ddmObj->indices[j+1];
          WORD c = ddmObj->indices[j+2];
          if(a != b && a != c && b != c)
          {
            if(j%2==0) polygons.push_back(SimpleFace(a, b, c));
            else       polygons.push_back(SimpleFace(b, a, c));
          }
        }

        for(DWORD j=0, numPolygons = polygons.size(); j<numPolygons; j++)
        {
          WORD a = (polygons[j].a);
          WORD b = (polygons[j].b);
          WORD c = (polygons[j].c);
          if(j!=0)
            matSplit[0].Indices.push_back(b);
          matSplit[0].Indices.push_back(b);
          matSplit[0].Indices.push_back(a);
          matSplit[0].Indices.push_back(c);
          if(j+1!=numPolygons)
          {
            matSplit[0].Indices.push_back(c);
        ...
        /*for(DWORD j=2; j<ddmObj->numIndices; j++)
        {
          if((j%2)==0)
          {
            matSplit[0].Indices[j] = ddmObj->indices[j-2];
            matSplit[0].Indices[j-2] = ddmObj->indices[j-1];
            matSplit[0].Indices[j-1] = ddmObj->indices[j];
          }
          else
          {
            matSplit[0].Indices[j-1] = ddmObj->indices[j-1];
            matSplit[0].Indices[j-2] = ddmObj->indices[j-2];
            matSplit[0].Indices[j] = ddmObj->indices[j];
          }
          
          
        }*/
        //Vertex* vertices = new Vertex[ddmObj->numVertices];
        first=false;
        splitCount=0;
        ...
        for(DWORD k=0; k<numObjects; k++)
        {
          if(meshIndices[k]==i)
          {
            mesh->GetCollision()->AddVertices(positions[k], &vertices.front(), vertices.size(), colours);
        ...
        {
          mesh->GetCollision()->AddVertices(positions[numObjects], &vertices.front(), vertices.size(), colours);
          Vertex bboxMin, bboxMax;
          Vertex vertex;
          for(DWORD j=0; j<ddmObj->numVertices; j++)
          {
...
inte:
  ...
  else
  {
        ...
        case 6:
          ...
          if((mesh = GetMesh(name,j)))
          {
            ...
            if(startNode==NULL)
            {
              ...
              sprintf(chc,"%s_StartGap",name.GetString());
              shatter = Checksum(chc).checksum;
            ...
            while(((WORD*)node)[0]==0x00C9)
            {
              ...
              {
                /*fe=false;
                //oldMesh->node = &this->nodes.back();
                sprintf(chc,"%s_StartGap",oldName.GetString());
                shatter = Checksum(chc).checksum;

                GetNode(oldName)->SetTrigger(shatter);
                //memcpy(&StartGap[12],&gapId,4);
                triggers.push_back(*(Checksum*)&shatter);

                WORD id = ((WORD*)(node+4))[0];

                for(DWORD i=0; i<803; i++)
                {
                if(id==Th2xGaps[i].id)
                {
                KnownScripts.push_back(KnownScript(*(Checksum*)&shatter, NULL, 0));
                KnownScript & script = KnownScripts.back();
                switch(Th2xGaps[i].type)
                {
                case 0x5:
                memcpy(&EndGap[12],&gapId,4);
                memcpy(&EndGap[73],&Th2xGaps[i].score,2);
                strcpy((char*)&EndGap[27], Th2xGaps[i].text);
                ...
                memcpy(&EndGapWithLoop[120],&Th2xGaps[i].score,2);
                strcpy((char*)&EndGapWithLoop[57],Th2xGaps[i].text);
                ...
                */
                /*gapType = GetGapType(((WORD*)(node+4))[0], (char*)&EndGap[27]);
                if(!gapType)
                {
                sprintf((char*)&EndGap[27], "%08X",name);
                memcpy(&StartGap[24], &Checksum("PURE_AIR"),4);
                memcpy(&EndGap[12],&gapId,4);
                /*Checksum("PURE_AIR");
                StartGap[24] = 0x60;
                StartGap[25] = 0x64;
                ...
                {
                memcpy(&StartGap[24], &Checksum("PURE_MANUAL"), 4);
                memcpy(&EndGap[12],&gapId,4);
                }
                else if(gapType==5)
                {
                memcpy(&StartGap[24], &Checksum("CANCEL_WALL"), 4);
                memcpy(&EndGap[12],&gapId,4);
                /*Checksum("CANCEL_WALL");
                StartGap[24] = 0x99; 
                StartGap[25] = 0xE8;
                StartGap[26] = 0xE4;
                StartGap[27] = 0x87;//
                ...
                //sprintf(chc, "Started_%s",(char*)&EndGap[27]);
                if(gapType==2)
                {
                memcpy(&StartGap[24], &Checksum("CANCEL_MANUAL"),4);
                memcpy(&EndGapWithLoop[29],&gapId,4);
                strcpy((char*)&EndGapWithLoop[57],(char*)&EndGap[27]);
                sprintf(chc, "StartedGap%d",gapIndex-1);
                EndId = Checksum(chc);
                globals.push_back(EndId);
                memcpy(&IsTrue[7], &EndId, 4);
                ...
                KnownScripts.push_back(KnownScript(*(Checksum*)&shatter, (void*)IsTrue, sizeof(IsTrue)));
                KnownScripts.back().Append(sizeof(StartGap), sizeof(StartGap), StartGap);//AppendScript(KnownScripts.size()-1, sizeof(StartGap), sizeof(StartGap), StartGap);
                KnownScripts.back().Append(sizeof(EndIF), sizeof(EndIF), EndIF);
                }
                else
                KnownScripts.push_back(KnownScript(*(Checksum*)&shatter, (void*)StartGap, sizeof(StartGap)));*/
              }
              else
              {
                /*memcpy(&StartGap[12],&gapId,4);
                ...
                AppendScript(mesh->node->Trigger, sizeof(IsTrue), sizeof(IsTrue), IsTrue);
                AppendScript(mesh->node->Trigger, sizeof(StartGap), sizeof(StartGap), StartGap);
                AppendScript(mesh->node->Trigger, sizeof(EndIF), sizeof(EndIF), EndIF);
                }
                else
                AppendScript(mesh->node->Trigger, sizeof(StartGap), sizeof(StartGap), StartGap);*/
                WORD id = ((WORD*)(node+4))[0];
                ...
                for(DWORD i=0; i<803; i++)
                {
                  if(id==Th2xGaps[i].id)
                  {
                    KnownScript* script = GetScript(startNode->Trigger.checksum);
                    if(script)
                    {
                      switch(Th2xGaps[i].type)
                      {
              ...
              if((mesh = GetMesh(name,j)))
              {
                ...
                if(meshNode==NULL || meshNode->Trigger.checksum==0)
                {
                  if(!name.GetString())
                  {
                    sprintf(chc,"Mesh%d",j);
                  ...
                  sprintf(chc,"%s_StartGap",name.GetString());
                  KnownScript* script = GetScript(Checksum(chc).checksum);
                  if(script)
                  {
                    if(gapType==2)
                    {
                ...
                else
                {
                  if(gapType==2)
                  {
                    ...
                    gapId = Checksum(chc);
                    memcpy(&SpawnEndGapWithLoop[6], &gapId, 4);
                    AppendScript(meshNode->Trigger, sizeof(SpawnEndGapWithLoop), sizeof(SpawnEndGapWithLoop), SpawnEndGapWithLoop);
                    triggers.push_back(*(Checksum*)&gapId);
                    //sprintf(chc,"StartedGap%d",gapIndex-2);
                    memcpy(&EndGapWithLoop[151], &EndId, 4);
                    KnownScripts.push_back(KnownScript(*(Checksum*)&gapId, (void*)EndGapWithLoop, sizeof(EndGapWithLoop)));
                  ...
                  {
                    AppendScript(meshNode->Trigger, sizeof(EndGap), sizeof(EndGap), EndGap);
                  }
                }
              }
            }
    ...
    for(DWORD i=0, numMeshes = this->meshes.size(); i<numMeshes; i++)
    {
      ...
      {
      if(this->nodes[j].Class.checksum == 0xDABD3086 && this->nodes[j].Name.checksum == this->meshes[i].GetName().checksum)
      {
      found = true;
      break;
      }
    ...
    for(DWORD i=0, numNodes = this->nodes.size(); i<numNodes; i++)
    {
      if(this->nodes[i].TrickObject)
      {
        ...
        for(DWORD j=0; j<nodes[i].Links.size(); j++)
        {
          ...
          if(link.Class.checksum == 0xDABD3086 && !link.Cluster.checksum)
          {
            link.Cluster = this->nodes[i].Cluster;
            for(DWORD k=0; k<link.Links.size(); k++)
            {
              if(!nodes[link.Links[k]].Cluster.checksum && (nodes[link.Links[k]].Class.checksum == 0xDABD3086 || nodes[link.Links[k]].Class.checksum == 0x8E6B02AD))
                nodes[link.Links[k]].Cluster = link.Cluster;
            }
          }
          else if(link.Class.checksum == 0x8E6B02AD && !link.Cluster.checksum)
          {
            link.Cluster = this->nodes[i].Cluster;
            for(DWORD k=0; k<link.Links.size(); k++)
            {
      ...
      /*else if(this->nodes[i].Class.checksum==0xDABD3086)
      {
    ...
    for(DWORD i=0, numNodes = this->nodes.size(); i<numNodes; i++)
    {
      if(!this->nodes[i].Cluster.checksum && this->nodes[i].Class.checksum == 0x8E6B02AD)
      {
        ...
        for(DWORD j=0; j<this->nodes[i].Links.size(); j++)
        {
          ...
          if(link.TrickObject && link.Class.checksum == 0xDABD3086)
          {
            ...
            else if(this->nodes[i].Cluster.checksum)
            {
              ...
              if(tmpMesh)
              {
                ...
                for(DWORD k=0; k<this->nodes[i].Links.size(); k++)
                {
                  ...
                  if(meshLink.Class.checksum == 0x8E6B02AD)
                  {
      ...
      else if(this->nodes[i].Class.checksum == 0xDABD3086 && this->nodes[i].CreatedAtStart)
      {
        if(this->nodes[i].LinksTo(0x1806DDF8, &this->nodes[i].Links[0], &this->nodes.front()))
    ...
    for(DWORD i=0, numNodes = this->nodes.size(); i<numNodes; i++)
    {
      if(this->nodes[i].Cluster.checksum && this->nodes[i].Class.checksum == 0x8E6B02AD)
      {
        for(DWORD j=0; j<this->nodes[i].Links.size(); j++)
        {
          ...
          if(!link.Cluster.checksum && link.Class.checksum == 0x8E6B02AD)
          {
            link.Cluster=this->nodes[i].Cluster;
          }
        }
