...
DWORD SuppurtedScripts[] = {
	Checksums::Sk3_TeleportToNode,
	Checksums::Obj_MoveToNode,
	Checksums::Obj_FollowPathLinked,
	Checksums::Obj_StickToGround,
	Checksums::Obj_SetPathVelocity,
	Checksums::Obj_MoveToPos,
	Checksums::Obj_MoveToRelPos,
	Checksums::Obj_PlaySound,
	Checksums::PlaySound,
	Checksums::Kill,
	Checksums::Shatter,
	Checksums::Create,
...
_declspec (noalias) const BYTE const* __restrict Scene::ParseScript(register const BYTE const* __restrict pFile, KnownScript * *pScript, vector<KnownScript> & scripts, vector<Script> & sounds, bool THUG, vector<Checksum> * extraTriggers)
{
	...
	case Checksums::Obj_MoveToNode:
	case Checksums::Obj_MoveToPos:
	case Checksums::Obj_MoveToRelPos:
	case Checksums::Obj_FollowPathLinked:
	case Checksums::Obj_StickToGround:
	case Checksums::Obj_SetPathVelocity:
		script->Script(qbKey);
		pFile = script->AddParams(pFile, THUG);
		script->EndScript();
		break;
		...
		case 0x16:
			if (node)
			{
				...
				case Checksums::TriggerScript:
					...
					if (*(DWORD*)pFile)
					{
						printf("Adding Trigger %X\n", *(DWORD*)pFile);
						if(node->Trigger.checksum)
						  MessageBox(0, "", node->Trigger.GetString(), 0);
						node->Trigger = *(Checksum*)pFile;
						pFile += sizeof(Checksum);
						triggers.push_back(node->Trigger);
					}
